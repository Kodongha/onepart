<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
<jsp:include page="../vote/vote_include.jsp"></jsp:include>
<jsp:include page="../vote/vote_detail_include.jsp"></jsp:include>
<div>
					<table style="width:100%">
						<tr>
							<td style="width:80%"></td>
							<td style="width:10%"><h5>미완료</h5></td>
							<td style="width:10%"><a class="btn btn-white" style="width: 100%" id="modifyGeneral">수정하기</a></td>
						</tr>
					</table>
				</div>		
			</div>
			<hr>
			<br>
		</div>
				<!-- 후보정보 div -->
				<div style="width:95%; margin:0 auto;">
					<h4>후보정보</h4>
					<br>
					<!-- 각 후보정보 div -->
					<div>
						<table style="width:100%">
							<tr>
								<td><h4>후보 1. 지용이네 페인트</h4></td>
								<td style="width:20%"><a class="btn btn-white" style="background:skyblue; width: 100%;">선택</a></td>
							</tr>
							<tr>
								<td colspan="2">
									<h6>상세정보</h6>
									<p>
									방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는
									방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는	
									</p>
								</td>
							</tr>
						</table>
					</div>
					<br>
					<br>
					<div>
						<table style="width:100%">
							<tr>
								<td><h4>후보 2. Vi-fi 페인트공장</h4></td>
								<td style="width:20%"><a class="btn btn-white" style="background:orangered; width: 100%;">미선택</a></td>
							</tr>
							<tr>
								<td colspan="2">
									<h6>상세정보</h6>
									<p>
									방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는
									방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는	
									</p>
								</td>
							</tr>
						</table>
					</div>
					<br>
					<br>
					<div>
						<table style="width:100%">
							<tr>
								<td><h4>후보 3. KH 페인트</h4></td>
								<td style="width:20%"><a class="btn btn-white" style="background:orangered; width: 100%;">미선택</a></td>
							</tr>
							<tr>
								<td colspan="2">
									<h6>상세정보</h6>
									<p>
									방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는
									방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는 방지하는 소금이라 할지니 인생에 가치를 주는	
									</p>
								</td>
							</tr>
						</table>
					</div>
					<br>
					<br>
				</div>
			</div>
		</div>
		<!-- end #content -->
</body>
</html>