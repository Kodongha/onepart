...
public class NoticeController {
	...
	public ModelAndView moveNoticeWrite(ModelAndView mv, int noticeSeq, String noticeTitle, String noticeContent, Date noticeEnrollDt  ) {
		System.out.println("/moveNoticeWrite");

		NoticeVo n = new NoticeVo();

		n.setNoticeSeq(noticeSeq);
		n.setNoticeTitle(noticeTitle);
		n.setNoticeContent(noticeContent);
		n.setNoticeEnrollDt(noticeEnrollDt);
		
		//확인
		System.out.println("controller NoticeVo :" + n.toString());
		
		ArrayList<NoticeVo> list = service.insertNoticeVo();

		mv.addObject("list", list);
		mv.setViewName("/resident/my_apartment/notice/notice_write");

		return mv;
	}

	//공지사항 수정
	@RequestMapping( value= "/resident/moveNoticeUpdate", method= {RequestMethod.POST, RequestMethod.GET})
	public ModelAndView moveNoticeUpdate(@RequestParam(value="noticeSeq")int noticeSeq, @ModelAttribute("noticeVo") NoticeVo noticeVo) {
		...
		mv.addObject("list", list);
		mv.setViewName("/resident/my_apartment/notice/notice_update");
